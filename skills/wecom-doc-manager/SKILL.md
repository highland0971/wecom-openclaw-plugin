...
使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc <tool_name> '<json_params>'` 调用指定技能
...
- 首次调用（不传 task_id）：使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc get_doc_content '{"docid": "DOCID", "type": 2}'`
- 轮询（携带上次返回的 task_id）：使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc get_doc_content '{"docid": "DOCID", "type": 2, "task_id": "xxx"}'`
- 或通过 URL：使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc get_doc_content '{"url": "https://doc.weixin.qq.com/doc/xxx", "type": 2}'`
...
- 使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc create_doc '{"doc_type": 3, "doc_name": "项目周报"}'`
- 使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc create_doc '{"doc_type": 10, "doc_name": "任务跟踪表"}'`
...
使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc edit_doc_content '{"docid": "DOCID", "content": "# 标题\n\n正文内容", "content_type": 1}'`
...
- 使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc smartpage_create '{"title": "项目概览", "pages": [{"page_title": "需求文档", "content_type": 1, "page_filepath": "/path/to/requirements.md"}]}'`
...
- 使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc smartpage_export_task '{"docid": "DOCID", "content_type": 1}'`
...
- 使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc smartpage_get_export_result '{"task_id": "TASK_ID"}'`
...
1. **读取文档/智能表格内容**（URL 含 `/doc/` 或 `/smartsheet/`） → 使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc get_doc_content '{"docid": "DOCID", "type": 2}'`，若 `task_done` 为 false 则携带 `task_id` 继续轮询
2. **创建新文档** → 使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc create_doc '{"doc_type": 3, "doc_name": "文档名"}'`，保存返回的 docid
3. **编辑文档** → 先 get_doc_content 了解当前内容，再 edit_doc_content 覆写
4. **创建智能主页** → 使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc smartpage_create '{"title": "标题", "pages": [{"page_title": "子页面", "content_type": 1, "page_filepath": "/path/to/file.md"}]}'`，保存返回的 docid
5. **获取智能主页内容**（URL 含 `/smartpage/`，异步两步）：
   - **第一步**：使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc smartpage_export_task '{"docid": "DOCID", "content_type": 1}'`，获取 `task_id`
   - **第二步**：使用 `wecom_mcp` tool 调用 `wecom_mcp openapi doc smartpage_get_export_result '{"task_id": "TASK_ID"}'`，若 `task_done` 为 `false` 则继续轮询，直到 `task_done` 为 `true`
