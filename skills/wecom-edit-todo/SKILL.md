...
使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo create_todo '<json格式的入参>'`
...
使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo create_todo '{"content": "<待办的内容>", "remind_time": "2025-06-01 09:00:00"}'`
...
使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo update_todo '<json格式的入参>'`
...
使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo update_todo '{"todo_id": "TODO_ID", "content": "<待办的内容>", "remind_time": "2025-07-01 09:00:00"}'`
...
使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo delete_todo '<json格式的入参>'`
...
使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo delete_todo '{"todo_id": "TODO_ID"}'`
...
使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo change_todo_user_status '<json格式的入参>'`
...
使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo change_todo_user_status '{"todo_id": "TODO_ID", "user_status": 2}'`
...
2. 第二步：创建待办并分派，使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo create_todo '{"content": "<待办的内容>", "follower_list": {"followers": [{"follower_id": "zhangsan", "follower_status": 1}]}, "remind_time": "2025-03-24 09:00:00"}'`
...
1. 第一步：通过 wecom-get-todo-list 获取待办列表，找到目标待办的 todo_id。使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo get_todo_list '{}'`
2. 第二步：通过 wecom-get-todo-detail 获取详情，确认是目标待办。使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo get_todo_detail '{"todo_id_list": ["TODO_ID"]}'`
3. 第三步：确认后，将待办状态改为已完成。使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo update_todo '{"todo_id": "TODO_ID", "todo_status": 0}'`
...
1. 第一步：通过 wecom-get-todo-list 获取待办列表，找到目标待办的 todo_id。使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo get_todo_list '{}'`
2. 第二步：通过 wecom-get-todo-detail 获取详情，确认是目标待办。使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo get_todo_detail '{"todo_id_list": ["TODO_ID"]}'`
3. 第三步：确认后，变更当前用户的参与状态为已完成。使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo change_todo_user_status '{"todo_id": "TODO_ID", "user_status": 2}'`
...
1. 第一步：查找目标待办。使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo get_todo_list '{}'`，再使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo get_todo_detail '{"todo_id_list": ["TODO_ID_1", "TODO_ID_2"]}'`
2. 第二步：确认目标后更新。使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo update_todo '{"todo_id": "TODO_ID", "remind_time": "2025-03-28 09:00:00"}'`

### 删除待办

用户问："删掉'代码评审'那个待办"

1. 第一步：查找目标待办。使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo get_todo_list '{}'`，再使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo get_todo_detail '{"todo_id_list": ["TODO_ID"]}'`
2. 第二步：向用户确认后删除。使用 `wecom_mcp` tool 调用 `wecom_mcp openapi todo delete_todo '{"todo_id": "TODO_ID"}'`
